...
function pclookuptable {
        ...
        $BiosS = (Get-WmiObject -Class win32_bios).serialnumber
        $MoboM = Get-WmiObject -Class win32_baseboard | format-Table Product,Manufacturer
        $ProcN = (Get-WmiObject -Class Win32_Processor).name
        $RAMST = ramlook
        $HdS = Get-WmiObject -Class MSFT_PhysicalDisk -Namespace root\Microsoft\Windows\Storage | Format-Table @{Name="GB"; Expression={[math]::round($_.size/1GB, 2)}},friendlyname
        $OSN = (Get-WMIObject -Class win32_operatingsystem).name
        $MoboS = (Get-WmiObject -Class win32_baseboard).serialnumber
        $GPUN = (Get-WmiObject -Class Win32_VideoController).description
        $LogN = (Get-WmiObject -Class Win32_Process -Filter 'Name="explorer.exe"').
            GetOwner().
            User
        $ModelMa = (Get-WmiObject -Class:Win32_ComputerSystem).Manufacturer
        $ModelMO = (Get-WmiObject -Class:Win32_ComputerSystem).Model
        $OStype = (get-wmiobject win32_operatingsystem).OSArchitecture
        $Prnt = (Get-WMIObject Win32_Printer).Name
        $NetIDs = Get-WmiObject -Class "Win32_NetworkAdapterConfiguration" -Filter "IpEnabled = TRUE"
